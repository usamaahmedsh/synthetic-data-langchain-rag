...
# Llama.cpp server launcher for HPC (CPU-only, auto-download model, background)
...
# Context and parallel settings (more conservative for HPC CPU)
...
download_model() {
  ...
  if command -v curl >/dev/null 2>&1; then
    ...
    echo "  Please install one of them or pre-download the model manually."
    exit 1
  fi
...
echo "════════════════════════════════════════"
echo "  LLaMA.cpp Server Launcher (HPC CPU)"
echo "════════════════════════════════════════"
echo ""
...
echo "  GPU layers:        $GPU_LAYERS (0 = CPU only)"
...
# Run in background, redirecting stdout/stderr
# Use nohup so it survives if shell exits inside the job script
nohup "$LLAMA_SERVER" \
  --model "$MODEL_PATH" \
  --host "$HOST" \
  --port "$PORT" \
...
echo "If everything is OK, your Python pipeline can now call:"
