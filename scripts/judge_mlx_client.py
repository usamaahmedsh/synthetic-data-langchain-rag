...
def build_judge_prompt(
    ...
    """Optimized prompt - more concise while maintaining clarity."""
    rubric_lines = [f"{it['label']}: {it['question']}" for it in spec]
    rubric = "\n".join(rubric_lines)
    labels = ", ".join([f'"{it["label"]}": <0..1>' for it in spec])
    
    return f"""Score this query. Return ONLY JSON with scores [0,1] for each rubric and total_score [0,1].
...
JSON: {{"scores": {{{labels}}}, "total_score": <0..1>}}""".strip()
...
async def call_llama_json_async(
    ...
    Expects the model to return a single JSON object string.[web:2][web:30]
    """
...
class JudgeQueriesMLX:
    ...
    def __init__(
        ...
        *,
        ...
        # Configure local ChatLlamaCpp.[web:2][web:14][web:30]
